...
async function deleteSource(sourceId) {
  const btn = document.querySelector(`.btn-del[data-id="${sourceId}"]`);
  const sourceName = btn?.dataset.name || "Bu kaynak";
  
  if (!confirm(`${sourceName} kaynağını ve buna bağlı TÜM haberleri silmek istediğinizden emin misiniz?\n\nBu işlem geri alınamaz!`)) {
    return;
  }
  ...
  try {
    ...
    okMsg(json.message || "Kaynak başarıyla silindi.");
    await loadSources(); // Listeyi yenile
  } catch (err) {
    console.error("Kaynak silme hatası:", err);
